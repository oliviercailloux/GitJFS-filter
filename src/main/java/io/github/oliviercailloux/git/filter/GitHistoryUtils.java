...
//		}
		...
		final Graph<ObjectId> graph = Graphs.transpose(GitHistoryUtilsSupport.asGraph(c -> Arrays.asList(c.getParents()), allCommits));
	...
	public static GitHistory getHistory(GitFileSystem gitFs) throws IOException {
		...
		try {
			final ImmutableGraph<ObjectId> graphOfIds = GitHistoryUtilsSupport.asImmutableGraph(graphOfPaths,
