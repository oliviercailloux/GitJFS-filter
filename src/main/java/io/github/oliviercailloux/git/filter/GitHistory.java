...
public class GitHistory {
  ...
  /**
   * @return the nodes of the {@link #getGraph() graph} that have no children (no successor);
  ...
  public GitHistory filter(Predicate<ObjectId> predicate) {
    /*
     * Two choices are reasonable here. Either I just compute the induced subgraph, or I try not to
     * disconnect the graph. E.g., with a ← b ← c, filtering out b will disconnect the graph,
     * whereas I could instead return a ← c. The drawback of this last solution is that it is
     * difficult to implement, and it changes the parent relation.
     */
    final ImmutableSet<ObjectId> kept =
        graph.nodes().stream().filter(predicate).collect(ImmutableSet.toImmutableSet());
