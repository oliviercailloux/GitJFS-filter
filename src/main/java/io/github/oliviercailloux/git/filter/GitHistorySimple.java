...
/**
 * A history of commits. It can use the author dates, the commit dates, or dates from any other
 * source. It guarantees that every node in the graph has an associated date.
 * <p>
 * Longer term improvement: we’d better traficate the commit itself to change its date if we prefer
 * the push date (or any other source), and let clients use always the commit date. Then this class
 * would serve to filter a gitfs according to the commit date.
 * <p>
 ...
 * relation, it should not represent twice the same logical commit.
 * <p>
 * OR this notion should be extended with methods to represent the refs as well, that would not
 * appear in the graph.
 *
 * <p>
 * Idea: add getRefsMatching(TPredicate≤GitPathRoot> refPredicate): ImmutableSet≤GitPathRoot>.
 *
 * static filter(Set≤GitPathRoot>, TPredicate≤GitPath>): ImmutableSet≤GitPath>. NB I can use this to
 * filter on oids in order to implement anyMatch if nothing better. But what about
 * allMatchAndExists?
 *
 * <p>
 ...
 * permits to filter on commits that have zero files, for example (using Files.list); and
 * getCommitDate() should use the same param as filter because we might want to filter by date.
 * <li>Remains not easy: checks whether there exists some branch in which the file "somefile"
 * satisfies some predicate. But it can be done through #getRefs() and it is an edge case.
 * <li>FAILS because we want: paths in "origin" satisfying x. Can’t be done with predicate on all
 * paths: we need to let "origin/" pass, otherwise it stops there; but then even if no file satisfy;
 * we end up with a non empty set. OR we let the predicate pass origin iff it contains afile
...
public class GitHistorySimple {
   ...
   */
  public static GitHistorySimple create(GitFileSystem fs, Map<ObjectId, Instant> dates)
      throws IOException {
    return new GitHistorySimple(fs, dates);
  }

  /**
   * dates its keyset must contain all nodes of the graph.
   */
  public static GitHistorySimple usingCommitterDates(GitFileSystem fs) throws IOException {
    ...
    final ImmutableMap<Commit, Instant> dated =
        CollectionUtils.toMap(graph.nodes(), c -> c.committerDate().toInstant());
  ...
  private GitHistorySimple(GitFileSystem fs, Map<ObjectId, Instant> dates) throws IOException {
    this.fs = fs;
    graph = fs.graph();
    final ImmutableSet<ObjectId> commits =
        graph.nodes().stream().map(p -> p.getCommit().id()).collect(ImmutableSet.toImmutableSet());
    final ImmutableSet<ObjectId> commitsWithNoDates =
        Sets.difference(commits, dates.keySet()).immutableCopy();
  ...
  /**
   * The parents from which everything starts, that is, the smallest set of nodes from which all
   * nodes are reachable by following the “successors” (children) relation; equivalently, the
   * starting points in time of the git history, that is, the nodes that have no predecessor.
   * <p>
  ...
  public ImmutableSet<GitPathRootShaCached> roots() {
    if (roots == null) {
      /*
       * We could start from any given node and simply follow the predecessor (parents) relation,
       * but that finds only one root.
       */
  ...
  /**
   * @return the nodes of the {@link #graph() graph} that have no children (no successor);
   *         equivalently, the smallest set of nodes such that starting from any node and following
   *         the “successors” (children) relation necessarily ends in the set.
   *
   *         return empty iff the graph is empty.
   */
  ...
  /**
   * @throws IllegalArgumentException iff the given path corresponds to no node of the
   *         {@link #graph() graph}.
   */
  ...
  /**
   * @throws IllegalArgumentException iff the given commit id corresponds to no node of the
   *         {@link #graph() graph}.
   */
  ...
  /**
   * @return a map whose key set corresponds to the nodes of the {@link #graph() graph}
   */
  ...
  /**
   * Returns a file system that only shows paths whose timestamp match the given filter.
   *
  ...
  /**
   * Returns a git history that only shows paths whose timestamp match the given filter.
   *
   * @param filter indicates which elements should be kept
   * @return a filtering file system
   ...
   * @throws IOException TODO to remove
   */
  public GitHistorySimple filteredCommits(TPredicate<Commit, IOException> filter)
      throws IOException {
